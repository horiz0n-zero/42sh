...
/*   Updated: 2018/01/04 17:10:37 by kstoian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const t_redinfo	g_infos[] =
{
	...
	[TYPE_AR] = {0, 0, STDIN_FILENO, STDERR_FILENO, red_and},
	[TYPE_LA] = {0, 0, STDOUT_FILENO, STDERR_FILENO, red_and},
	[TYPE_IO] = {O_RDWR | O_APPEND | O_CREAT, R_PATH | 2, STDIN_FILENO,
		STDOUT_FILENO, NULL},
	[TYPE_RA] = {0, 0, STDIN_FILENO, STDERR_FILENO, red_and},
	[TYPE_AL] = {0, 0, STDOUT_FILENO, STDERR_FILENO, red_and}
};
...
void					foreground_redirections(t_dispatch *const dispatch)
{
	...
	{
		if (dispatch->foreground.collection.ccmd->red)
		{
			ptr = dispatch->foreground.collection.ccmd->red;
			while (ptr && ptr->next)
				ptr = ptr->next;
			ptr->next = new_red();
			ptr = ptr->next;
		...
		{
			dispatch->foreground.collection.ccmd->red = new_red();
			ptr = dispatch->foreground.collection.ccmd->red;
		}
	}
	else
